...
| `NodeRaft.fs` | Per-RPC-type handler functions (`handleRequestVote`, `handleRequestVoteResponse`, `handleAppendEntries`, `handleAppendEntriesResponse`, `handleInstallSnapshot`, `handleInstallSnapshotResponse`) — each handles exactly one Raft RPC type, calling the corresponding pure function then `saveIfChanged` and `sendResponse`. `handleRaftRPC` orchestrates: dispatches through per-type handlers, then applies committed entries, auto-snapshot, config finalization, broadcasts heartbeat on leader promotion, computes timer actions, and updates pending reads. |
| `NodeLocal.fs` | `handleLocalMessage` — dispatches `ClientCommand`, `AddPeer`, `RemovePeer` via `dispatchLocalMessage`. Each branch replies directly to the caller via the function-type callback embedded in the message. On leader: appends entries, broadcasts to peers, applies committed entries, auto-snapshots, and finalizes configuration inline (explicit pipeline, no hidden micro-orchestrator). On non-leader: returns `Redirect` with leader info. Also broadcasts heartbeat on leader promotion, processes pending reads, and returns `MessageResult` with timer actions. |
